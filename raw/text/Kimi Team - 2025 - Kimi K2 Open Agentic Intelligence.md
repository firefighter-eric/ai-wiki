...
- Source HTML: `raw/html/Kimi Team - 2025 - Kimi K2 Open Agentic Intelligence.html`
- Source URL: https://ar5iv.labs.arxiv.org/html/2507.20534
...
Let the input representation of a transformer layer be 𝐗\mathbf{X}. For each attention head hh, its query, key, and value projections are computed as

𝐐h=𝐗𝐖qh,𝐊h=𝐗𝐖kh,𝐕h=𝐗𝐖vh.\mathbf{Q}^{h}=\mathbf{X}\mathbf{W}_{q}^{h},\quad\mathbf{K}^{h}=\mathbf{X}\mathbf{W}_{k}^{h},\quad\mathbf{V}^{h}=\mathbf{X}\mathbf{W}_{v}^{h}.

where 𝐖q,𝐖k,𝐖v\mathbf{W}_{q},\mathbf{W}_{k},\mathbf{W}_{v} are model parameters. The attention output is:

𝐎h=softmax⁡(1d​𝐐h​𝐊h⊤)​𝐕h.\mathbf{O}^{h}=\operatorname{softmax}\left(\frac{1}{\sqrt{d}}\mathbf{Q}^{h}\mathbf{K}^{h\top}\right)\mathbf{V}^{h}.

We define the max logit, a per-head scalar, as the maximum input to softmax in this batch BB:

Smaxh=1d​max𝐗∈B⁡maxi,j⁡𝐐ih​𝐊jh⊤S_{\max}^{h}=\frac{1}{\sqrt{d}}\max_{\mathbf{X}\in B}\max_{i,j}\mathbf{Q}_{i}^{h}\mathbf{K}_{j}^{h\top}

where i,ji,j are indices of different tokens in a training sample 𝐗\mathbf{X}.

The core idea of QK-Clip is to rescale 𝐖k,𝐖q\mathbf{W}_{k},\mathbf{W}_{q} whenever SmaxhS_{\max}^{h} exceeds a target threshold τ\tau. Importantly, this operation does not alter the forward/backward computation in the current step — we merely use the max logit as a guiding signal to determine the strength to control the weight growth.

A naïve implementation clips all heads at the same time:

𝐖qh←γα​𝐖qh𝐖kh←γ1−α​𝐖kh\mathbf{W}_{q}^{h}\leftarrow\gamma^{\alpha}\mathbf{W}_{q}^{h}\qquad\mathbf{W}_{k}^{h}\leftarrow\gamma^{1-\alpha}\mathbf{W}_{k}^{h}

where γ=min⁡(1,τ/Smax)\gamma=\min(1,\tau/S_{\max}) with Smax=maxh⁡SmaxhS_{\max}=\max_{h}S_{\max}^{h}, and α\alpha is a balancing parameter typically set to 0.50.5, applying equal scaling to queries and keys.

However, we observe that in practice, only a small subset of heads exhibit exploding logits. In order to minimize our intervention on model training, we determine a per-head scaling factor γh=min⁡(1,τ/Smaxh)\gamma_{h}=\min(1,\tau/S_{\max}^{h}), and opt to apply per-head QK-Clip. Such clipping is straightforward for regular multi-head attention (MHA). For MLA, we apply clipping only on unshared attention head components:

- •

qC\textbf{q}^{C} and kC\textbf{k}^{C} (head-specific components): each scaled by γh\sqrt{\gamma_{h}}

- •

qR\textbf{q}^{R} (head-specific rotary): scaled by γh\gamma_{h},

- •

kR\textbf{k}^{R} (shared rotary): left untouched to avoid effect across heads.

1:for each training step tt do

2:  // 1. Muon optimizer step

3:  for each weight 𝐖∈ℝn×m\mathbf{W}\in\mathbb{R}^{n\times m} do

4:   𝐌t=μ​𝐌t−1+𝐆t\mathbf{M}_{t}=\mu\mathbf{M}_{t-1}+\mathbf{G}_{t} ⊳\triangleright 𝐌0=𝟎\mathbf{M}_{0}=\mathbf{0}, 𝐆t\mathbf{G}_{t} is the grad of 𝐖t\mathbf{W}_{t}, μ\mu is momentum

5:   𝐎t=Newton−Schulz⁡(𝐌t)⋅max⁡(n,m)⋅0.2\mathbf{O}_{t}=\operatorname{Newton-Schulz}(\mathbf{M}_{t})\cdot\sqrt{\max(n,m)}\cdot 0.2 ⊳\triangleright Match Adam RMS

6:   𝐖t=𝐖t−1−η​(𝐎t+λ​𝐖t−1)\mathbf{W}_{t}=\mathbf{W}_{t-1}-\eta\bigl{(}\mathbf{O}_{t}+\lambda\mathbf{W}_{t-1}\bigr{)} ⊳\triangleright learning rate η\eta, weight decay λ\lambda

7:  end for

8:  // 2. QK-Clip

9:  for each attention head hh in every attention layer of the model do

10:   Obtain SmaxhS_{\max}^{h} already computed during forward

11:   if Smaxh>τS_{\max}^{h}>\tau then

12:     γ←τ/Smaxh\gamma\leftarrow\tau/S_{\max}^{h}

13:     𝐖q​ch←𝐖q​ch⋅γ\mathbf{W}_{qc}^{h}\leftarrow\mathbf{W}_{qc}^{h}\cdot\sqrt{\gamma}

14:     𝐖k​ch←𝐖k​ch⋅γ\mathbf{W}_{kc}^{h}\leftarrow\mathbf{W}_{kc}^{h}\cdot\sqrt{\gamma}

15:     𝐖q​rh←𝐖q​rh⋅γ\mathbf{W}_{qr}^{h}\leftarrow\mathbf{W}_{qr}^{h}\cdot\gamma
...
To enhance mathematical reasoning capabilities, we rewrite high-quality mathematical documents into a “learning-note” style, following the methodology introduced in SwallowMath [15]. In addition, we increased data diversity by translating high-quality mathematical materials from other languages into English.
...
DeepSeek-V3 [10] sets the number of attention heads to roughly twice the number of model layers to better utilize memory bandwidth and enhance computational efficiency.
...
Our strategy leverages a combination of 16-way Pipeline Parallelism (PP) with virtual stages [28, 53, 38, 57, 47, 21], 16-way Expert Parallelism (EP) [39], and ZeRO-1 Data Parallelism [60].
...
By increasing the number of warm-up micro-batches, we can overlap EP all-to-all communication with computation under the standard interleaved 1F1B schedule [21, 53].
In comparison, DualPipe [10] doubles the memory required for parameters and gradients, necessitating an increase in parallelism to compensate.
...
Recomputation is applied to inexpensive, high-footprint stages, including LayerNorm, SwiGLU, and MLA up-projections [10].
...
We pre-trained the model with a 4,096-token context window using the MuonClip optimizer (Algorithm 1) and the WSD learning rate schedule [25], processing a total of 15.5T tokens.
...
We adopt K1.5 [35] and other in-house domain-specialized expert models to generate candidate responses for various tasks, followed by LLMs or human-based judges to perform automated quality evaluation and filtering.
...
To enhance our capability in tackling competition-level programming problems, we gather problems and their judges from both open-source datasets [27, 83] and synthetic sources. To ensure the diversity of the synthetic data and the correctness of reward signals, we incorporate high-quality human-written unit tests retrieved from pre-training data.
...
We adopt the policy optimization algorithm introduced in K1.5 [35] as the foundation for K2.
For each problem xx, we sample KK responses {y1,…,yk}\{y_{1},\dots,y_{k}\} from the previous policy πold\pi_{\mathrm{old}},
and optimize the model πθ\pi_{\theta} with respect to the following objective:

LRL​(θ)=𝔼x∼𝒟​[1K​∑i=1K[(r​(x,yi)−r¯​(x)−τ​log⁡πθ​(yi|x)πold​(yi|x))2]],\displaystyle L_{\mathrm{RL}}(\theta)=\mathbb{E}_{x\sim\mathcal{D}}\left[\frac{1}{K}\sum_{i=1}^{K}\left[\left(r(x,y_{i})-\bar{r}(x)-\tau\log\frac{\pi_{\theta}(y_{i}|x)}{{\pi}_{\mathrm{old}}(y_{i}|x)}\right)^{2}\right]\right]\,,

where r¯​(x)=1k​∑i=1kr​(x,yi)\bar{r}(x)=\frac{1}{k}\sum_{i=1}^{k}r(x,y_{i}) is the mean rewards of the sampled responses, τ>0\tau>0 is a regularization parameter that promotes stable learning.
As in SFT, we employ the Muon optimizer [33] to minimize this objective.
...
To prevent the potential forgetting of valuable, high-quality data during joint RL training, we curate a dataset comprising hand-selected, high-quality samples and integrate it into the RL objective through an auxiliary PTX loss [54].
...
Our system can complete a full parameter update for Kimi K2 with less than 30 seconds, a negligible duration for a typical RL training iteration.
...
For coding, we adopt LiveCodeBench v6 [31](questions from August 2024 to May 2025), OJBench [77], MultiPL-E [5], SWE-bench Verified [32, 84], TerminalBench [71], Multi-SWE-bench [86], SWE-Lancer [50], PaperBench [65], and Aider-Polyglot [16].
For tool use tasks, we evaluate performance on τ2\tau^{2}-Bench [3] and AceBench [6], which emphasize multi-turn tool-calling capabilities.
In reasoning, we include a wide range of mathematical, science and logical tasks: AIME 2024/2025, MATH-500, HMMT 2025, CNMO 2024, PolyMath-en, ZebraLogic [43], AutoLogi [91], GPQA-Diamond [61], SuperGPQA [13], and Humanity’s Last Exam (Text-Only) [56].
We benchmark the long-context capabilities on: MRCR555https://huggingface.co/datasets/openai/mrcr for long-context retrieval, and DROP [14], FRAMES [37] and LongBench v2 [2] for long-context reasoning.
For factuality, we evaluate FACTS Grounding [30], the Vectara Hallucination Leaderboard [73], and FaithJudge [68].
Finally, general capabilities are assessed using MMLU [23], MMLU-Redux [17], MMLU-Pro [76], IFEval [90], Multi-Challenge [64], SimpleQA [78], and LiveBench [80] (as of 2024-11-25).
...
For benchmarks with high per-question variance, we adopt repeated sampling kk times and average the results to obtain stable scores, denoted as Avg@k.
...
On multi-turn tool-use benchmarks, Kimi-K2-Instruct sets a new standard. It achieves 66.1 Pass@1 on τ2\tau^{2}-Bench and 76.5 on ACEBench, substantially outperforming all baselines. These results affirm its strength in grounded, controlled, and agent-driven tool orchestration across domains.
...
For general capabilities, we assess on MMLU [23], MMLU-Pro [76], MMLU-Redux [17], BBH [67], TriviaQA [34], SuperGPQA [13], SimpleQA [78], HellaSwag [88], AGIEval [89], GPQA-Diamond [61], ARC-Challenge [8], and WinoGrande [62].
For coding capabilities, we employ EvalPlus [45] (averaging HumanEval [7], MBPP [1], HumanEval+, and MBPP+), LiveCodeBench v6 [31], and CRUXEval [18].
For mathematical reasoning, we utilize GSM8K [9], GSM8K-Platinum [74], MATH [24], and CMATH [79].
For Chinese language capabilities, we evaluate on C-Eval [29], CMMLU [40], and CSimpleQA [22].
...
We choose Promptfoo666https://github.com/promptfoo/promptfoo to generate adversarial prompts and analyze the responses. By this way, we can evaluate model in a scalable ways.
...
In arXiv e-prints, 2025, pp. arXiv–2501
...
In International conference on machine learning, 2023, pp. 7480–7512
...
In arXiv preprint arXiv:2502.14739, 2025
...
In CoRR abs/1903.00161, 2019

arXiv: http://arxiv.org/abs/1903.00161
...
blog, https://aider.chat/docs/leaderboards/, 2025
...
In arXiv preprint arXiv:2401.03065, 2024
...
In arXiv preprint arXiv:2403.07714, 2025
...
In arXiv preprint arXiv:1806.03377, 2018
...
In URL https://arxiv. org/abs/2411.07140
...
In arXiv preprint arXiv:2009.03300, 2020
...
In arXiv preprint arXiv:2210.11610, 2022
...
In arXiv preprint arXiv:2501.12599, 2025
...
“Fact, Fetch, and Reason: A Unified Evaluation of Retrieval-Augmented Generation”, 2025
...
In Proceedings of Machine Learning and Systems 5, 2023, pp. 48–67
...
In Hugging Face repository 13.9, 2024, pp. 9
...
In arXiv preprint arXiv:2405.04434, 2024
...
In Advances in Neural Information Processing Systems 36, 2023, pp. 21558–21572
...
In arXiv preprint arXiv:2502.16982, 2025
...
arXiv: https://arxiv.org/abs/2502.05172
...
In arXiv preprint arXiv:2502.12115, 2025
...
In arXiv preprint arXiv:2407.03502, 2024
...
In arXiv preprint arXiv:2504.16891, 2025
...
In arXiv preprint arXiv:2401.10241, 2023
...
In arXiv preprint arXiv:2307.16789, 2023
...
In SC20: International Conference for High Performance Computing, Networking, Storage and Analysis, 2020, pp. 1–16
...
- [65]
Giulio Starace et al.

“PaperBench: Evaluating AI’s Ability to Replicate AI Research”

In arXiv preprint arXiv:2504.01848, 2025
...
In arXiv preprint arXiv:2505.04847, 2025
...
“The Llama 4 herd: The beginning of a new era of natively multimodal AI innovation — ai.meta.com” [Accessed 15-07-2025], https://ai.meta.com/blog/llama-4-multimodal-intelligence/
...
In arXiv preprint arXiv:2502.03461, 2025
...
In arXiv preprint arXiv:2411.04368, 2024
...
In URL https://arxiv. org/abs/2309.14322
...
In arXiv preprint arXiv:2406.12045, 2024
...
In arXiv preprint arXiv:2504.02605, 2025
...
In arXiv preprint arXiv:2304.06364, 2023
...
arXiv: https://arxiv.org/abs/2502.16906
...
We use TypeScript to express the tool declaration content, since TypeScript is a concise language with a comprehensive type system, able to express the types and constraints of tool parameters with brief text. The code 1 shows
an example for two simple tools in JSON format compatible with OpenAI’s chat completion API, as a comparison, the same tools defined in TypeScript (listed in Code 2) is much shorter.
...
For English scenarios, we leverage the Arena-Hard-Auto v2.0 benchmark, which use LLM-as-a-judge protocols to assess generation quality across diverse, open-ended prompts [42]. These evaluations cover a wide range of high-difficulty prompts and are widely recognized in the research community.
...
As shown in Figure 11, Kimi-K2-Instruct shows strong performance across all comparisons on Chinese in-house benchmarks. It outperforms ChatGPT-4o-latest with a 65.4% win rate, Claude Sonnet 4 with 64.6%, and DeepSeek-V3-0324 with 59.6%.
...
In addition to controlled evaluations, we also consider real-world user preference through public human assessments. As of July 17, 2025, Kimi-K2-Instruct ranked as the top open-source model and fifth overall on the LMSYS Arena leaderboard888https://lmarena.ai/leaderboard/text, based on over 3,000 blind votes from real users. Unlike LLM-as-a-judge protocols, this leaderboard reflects direct human preference on diverse, user-submitted prompts, providing a complementary perspective on practical model performance.
...
Initial 70000 steps: 12.7%12.7\% of attention heads triggered QK-Clip for at least once, clamping SmaxS_{\max} to 100100.

- •

Post-70000 steps: All heads at some point reduced their SmaxS_{\max} below 100100, rendering QK-Clip inactive.
...
Smax=maxi,j⁡(qi⋅kj)S_{\max}=\max_{i,j}\bigl{(}q_{i}^{\vphantom{\top}}\!\cdot k_{j}\bigr{)}

(1)

grows unboundedly during training. Since

|qi⋅kj|≤‖qi‖​‖kj‖≤‖xi‖​‖xj‖​‖𝐖q‖​‖𝐖k‖,|q_{i}\!\cdot\!k_{j}|\leq\|q_{i}\|\|k_{j}\|\leq\|x_{i}\|\|x_{j}\|\|\mathbf{W}_{q}\|\|\mathbf{W}_{k}\|,

(2)

and RMS-Norm keeps ‖xi‖​‖xj‖\|x_{i}\|\|x_{j}\| bounded, the phenomenon is primarily driven by the growing spectral-norm of 𝐖q\mathbf{W}_{q} or 𝐖k\mathbf{W}_{k}. Empirically, we found that Muon is more susceptible to logit explosion. We give our hypothesis below.

##### Structural difference in updates

Muon produces a weight update coming from the msign\mathrm{msign} operation; as a result, all singular values of the update matrix are equal — its effective rank is full.
...
Let the parameter matrix at step t−1t-1 have the singular value decomposition

𝐖t−1=∑iσi​ui​vi⊤\mathbf{W}_{t-1}=\sum_{i}\sigma_{i}\,u_{i}v_{i}^{\top}

(3)

We write the update matrices as

Δ​𝐖t\displaystyle\Delta\mathbf{W}_{t}
=∑jσ¯​u¯j​v¯j⊤\displaystyle=\sum_{j}\bar{\sigma}\,\bar{u}_{j}\bar{v}_{j}^{\top}

(4)

The next parameter update is therefore

𝐖t←∑iσi​ui​vi⊤+∑jσ¯​u¯j​v¯j⊤\displaystyle\mathbf{W}_{t}\leftarrow\sum_{i}\sigma_{i}u_{i}v_{i}^{\top}+\sum_{j}\bar{\sigma}\,\bar{u}_{j}\bar{v}_{j}^{\top}

(5)

In Muon, as both the weights and the updates have a higher effective rank than Adam, we hypothesize there is a higher probability for singular-vector pair ui​vi⊤u_{i}v_{i}^{\top} to align with u¯j​v¯j⊤\bar{u}_{j}\bar{v}_{j}^{\top}. This could cause the corresponding singular value of 𝐖t\mathbf{W}_{t} to increase additively.

##### Attention-specific amplification

Attention logits are computed via the bilinear form

qi⋅kj=(xi​𝐖q)⋅(xj​𝐖k).q_{i}\cdot k_{j}=(x_{i}\mathbf{W}_{q})\cdot(x_{j}\mathbf{W}_{k}).

(6)

The product 𝐖q​𝐖k⊤\mathbf{W}_{q}\mathbf{W}_{k}^{\top} squares the spectral norm, so any singular-value increase in either matrix is compounded. Muon’s tendency to enlarge singular values therefore translates into a higher risk of logit explosion.
...
As illustrated in Figure 13(a), a three-stage pipeline is introduced.
...
On NVIDIA H800 clusters, concurrent H2D and broadcast saturate the shared PCIe fabric, collapsing the three stages into a sequential procedure (Figure 13(b)).
We therefore adopt a simpler, two-stage scheme (Figure 13(c)): (1) All devices perform a single, synchronous H2D transfer. (2) The broadcast and reload proceed in parallel.
...
Generated on Tue Aug 5 18:03:53 2025 by LaTeXML
