...
- Source HTML: `raw/html/Sutskever, Vinyals, Le - 2014 - Sequence to Sequence Learning with Neural Networks.html`
- Source URL: https://ar5iv.labs.arxiv.org/html/1409.3215
...
their ability to sort N𝑁N ​​​​ N𝑁N-bit numbers using only 2
...
sequences. Given a sequence of inputs (x1,…,xT)subscript𝑥1…subscript𝑥𝑇(x_{1},\ldots,x_{T}), a
standard RNN computes a sequence of outputs (y1,…,yT)subscript𝑦1…subscript𝑦𝑇(y_{1},\ldots,y_{T}) by
iterating the following equation:

htsubscriptℎ𝑡\displaystyle h_{t}
=\displaystyle=
sigm​(Whx​xt+Whh​ht−1)sigmsuperscript𝑊hxsubscript𝑥𝑡superscript𝑊hhsubscriptℎ𝑡1\displaystyle\mathrm{sigm}\left(W^{\mathrm{hx}}x_{t}+W^{\mathrm{hh}}h_{t-1}\right)

ytsubscript𝑦𝑡\displaystyle y_{t}
=\displaystyle=
Wyh​htsuperscript𝑊yhsubscriptℎ𝑡\displaystyle W^{\mathrm{yh}}h_{t}
...
p​(y1,…,yT′|x1,…,xT)𝑝subscript𝑦1…conditionalsubscript𝑦superscript𝑇′subscript𝑥1…subscript𝑥𝑇p(y_{1},\ldots,y_{T^{\prime}}|x_{1},\ldots,x_{T}) where (x1,…,xT)subscript𝑥1…subscript𝑥𝑇(x_{1},\ldots,x_{T}) is an
input sequence and y1,…,yT′subscript𝑦1…subscript𝑦superscript𝑇′y_{1},\ldots,y_{T^{\prime}} is its corresponding output
sequence whose length T′superscript𝑇′T^{\prime} may differ from T𝑇T. The LSTM computes this
conditional probability by first obtaining the fixed-dimensional
representation v𝑣v of the input sequence (x1,…,xT)subscript𝑥1…subscript𝑥𝑇(x_{1},\ldots,x_{T}) given by
the last hidden state of the LSTM, and then computing the probability
of y1,…,yT′subscript𝑦1…subscript𝑦superscript𝑇′y_{1},\ldots,y_{T^{\prime}} with a standard LSTM-LM formulation whose
initial hidden state is set to the representation v𝑣v of
x1,…,xTsubscript𝑥1…subscript𝑥𝑇x_{1},\ldots,x_{T}:

p​(y1,…,yT′|x1,…,xT)=∏t=1T′p​(yt|v,y1,…,yt−1)𝑝subscript𝑦1…conditionalsubscript𝑦superscript𝑇′subscript𝑥1…subscript𝑥𝑇superscriptsubscriptproduct𝑡1superscript𝑇′𝑝conditionalsubscript𝑦𝑡𝑣subscript𝑦1…subscript𝑦𝑡1p(y_{1},\ldots,y_{T^{\prime}}|x_{1},\ldots,x_{T})=\prod_{t=1}^{T^{\prime}}p(y_{t}|v,y_{1},\ldots,y_{t-1})

(1)

In this equation, each p​(yt|v,y1,…,yt−1)𝑝conditionalsubscript𝑦𝑡𝑣subscript𝑦1…subscript𝑦𝑡1p(y_{t}|v,y_{1},\ldots,y_{t-1}) distribution
...
sentence a,b,c𝑎𝑏𝑐a,b,c to the sentence α,β,γ𝛼𝛽𝛾\alpha,\beta,\gamma, the LSTM is
asked to map c,b,a𝑐𝑏𝑎c,b,a to α,β,γ𝛼𝛽𝛾\alpha,\beta,\gamma, where α,β,γ𝛼𝛽𝛾\alpha,\beta,\gamma is the translation of a,b,c𝑎𝑏𝑐a,b,c. This way, a𝑎a is in close
proximity to α𝛼\alpha, b𝑏b is fairly close to β𝛽\beta, and so on, a
...
probability of a correct translation T𝑇T given the source sentence
S𝑆S, so the training objective is

1/|𝒮|​∑(T,S)∈𝒮log⁡p​(T|S)1𝒮subscript𝑇𝑆𝒮𝑝conditional𝑇𝑆1/|\mathcal{S}|\sum_{(T,S)\in\mathcal{S}}\log p(T|S)

where 𝒮𝒮\mathcal{S} is the training set. Once training is complete, we produce
translations by finding the most likely translation according to
the LSTM:

T^=arg⁡maxT⁡p​(T|S)^𝑇subscript𝑇𝑝conditional𝑇𝑆\hat{T}=\arg\max_{T}p(T|S)

(2)

We search for the most likely translation using a simple left-to-right
beam search decoder which maintains a small number B𝐵B of partial
...
increases the number of the hypotheses so we discard all but the B𝐵B
...
a threshold. For each training batch, we compute s=‖g‖2𝑠subscriptnorm𝑔2s=\left\|g\right\|_{2}, where g𝑔g is the gradient divided by 128. If s>5𝑠5s>5, we set
g=5​gs𝑔5𝑔𝑠g=\frac{5g}{s}.
...
responsible for multiplying by a 1000×200001000200001000\times 20000 matrix. The
...
In Journal of Machine Learning Research, pages 1137–1155,
2003.
...
In Arxiv preprint arXiv:1308.0850, 2013.
...
Master’s thesis, Institut fur Informatik, Technische
...
In Proc. 3rd Scandinavian Workshop on Algorithm Theory, 1992.
...
Proceedings of IEEE, 1990.

Generated on Mon Mar 18 06:30:59 2024 by LaTeXML
