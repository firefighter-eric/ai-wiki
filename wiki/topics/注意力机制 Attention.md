...
从当前 evidence base 看，本页可以稳定成立的核心判断是：**attention 的研究主线不是从一种标准形式演化到另一种更先进的标准形式，而是围绕不同瓶颈分化出了几类彼此只部分可比的路线。** `Vaswani et al.` 提供统一基线；`Longformer / BigBird / Reformer` 改连接图；`Linformer / Performer / Nyströmformer` 近似完整矩阵；`KDA` 用 fixed-size recurrent state 改写长序列状态；`MQA / GQA / MLA` 压缩模型级解码态；`CSA / HCA` 把长上下文 KV 压缩与稀疏选择组合到百万 token 场景；`FlashAttention` 重写一次 attention 的执行方式；`PagedAttention / RadixAttention` 则进入 serving runtime，分别处理 KV 的物理分页和跨请求前缀复用。
...
- **recurrent/linear attention 如何与 full global attention 共存**，以及 fixed-size state 节省了什么、又把复杂性转移到哪些训练与 serving 环节。
- **serving runtime 中的 KV cache 技术处于哪一层**：物理 block/page 管理、跨请求 prefix index 与模型级 `K/V` 头压缩不能混为一谈。
...
- **recurrent state 与混合全局 attention 层**：`Kimi K3` 的 `KDA` 用 channel-wise decay 与 delta-rule update 维护 fixed-size recurrent state，使主要 layers 的状态不随 sequence length 增长；但 K3 并未让 recurrent state 独自承担全部交互，而是每 3 个 KDA layers 插入 1 个 NoPE Gated MLA layer，并让最后一层保持全局 attention。这个层次的真实交换是：减少逐 token KV 增长，同时引入串行 state propagation、context-parallel scan、speculative rollback 和 checkpoint-aware prefix caching。它与 Performer 一类 kernel approximation 只在“线性 attention”宽口径下相邻，状态语义与系统约束并不相同。
...
- **serving runtime 的 KV 组织与复用层**：`PagedAttention` 把一个 sequence 的 logical KV blocks 映射到不连续 physical blocks，主要解决动态长度、连续预留与 fragmentation；`RadixAttention` 把已计算 token prefix 与 paged KV tensors 组织成 radix tree，主要解决跨请求 prefix matching、保留、淘汰与 locality-aware scheduling。二者不是互斥 attention 公式：SGLang 的 RadixAttention 本身使用 paged KV layout，而当前 vLLM 也在 paged block manager 上实现 hash-based Automatic Prefix Caching。这一层与 `MQA / GQA / MLA` 的区别在于，前者管理模型已经产生的 KV states，后者改变每个 token 需要产生多少或何种 KV 表示。
...
此外，`Attention Residuals` 虽带有 attention 名称，却作用于 network depth：它让一层选择性读取先前 layer/block representations，不是 token sequence attention。本页把它作为边界案例保留，而不把它混入长序列 attention 排名。
...
- **KDA 是否“消除了 KV cache”**：只能对 KDA layers 这样概括，且仍不准确。KDA 把随长度增长的 KV states 换成 fixed-size recurrent state；K3 同时保留 Gated MLA layers，其 KV cache 仍随序列增长。生产 prefix reuse 还必须让 MLA blocks 与 KDA checkpoints 在同一 token boundary 上一致命中。
...
- **`PagedAttention` 与 `RadixAttention` 是否是竞争关系**：不是严格的二选一。前者首先定义 KV tensors 的物理分页与 block table；后者首先定义 prefix 的语义索引、缓存生命周期与 locality policy。当前 SGLang 同时使用 paged layout 与 radix tree，当前 vLLM 也同时使用 paged blocks 与 hash-based prefix cache，因此稳定差异是数据结构与调度重心，而不是“有没有前缀缓存”。
