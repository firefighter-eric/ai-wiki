...
- `DeepSeek-V3.2` 与 `Kimi K3` 已把 thinking tool-use、不同 harness、长程 environment 与 verifier 写成 reasoning model 的重要训练接口，但当前知识库还没有正式 agent topic，因此这些内容只能暂时由本页承接。
- reasoning RL 的 **评测指标、可读性约束、monitorability、reward hacking 风险** 仍缺少独立研究支撑。K3 虽报告 hidden verifier、hacking detection 与 sandbox isolation，却不能单凭开发者报告证明长程 agent 已安全可控。
